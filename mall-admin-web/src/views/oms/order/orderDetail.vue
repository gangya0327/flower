<template> 
  <div class="detail-container">
    ...
    <el-card shadow="never" style="margin-top: 15px">
      <div class="operate-container">
        ...
        <div class="operate-button-container" v-show="order.status===1">
          <el-button size="mini" @click="showPayMoneyDialog">订单付款</el-button>
          <el-button size="mini" @click="showHandleFinishStuffDialog">材料办理确认</el-button>
          <!-- <el-button size="mini" @click="showUpdateReceiverDialog">修改收货人信息</el-button> -->
          <!-- <el-button size="mini" @click="showMessageDialog">发送站内信</el-button> -->
        ...
        <div class="operate-button-container" v-show="order.status===2">
          ...
          <!-- <el-button size="mini" @click="showMessageDialog">发送站内信</el-button> -->
          <el-button size="mini" @click="showPayMoneyDialog">订单付款</el-button>
          <el-button size="mini" @click="showAgreementSignDialog">协议签署确认</el-button>
          <!-- <el-button size="mini" @click="showCloseThisOrderDialog">关闭当前订单</el-button> -->
          <el-button size="mini" @click="showRefundOrderDialog">订单退款</el-button>
          <el-button size="mini" @click="showMarkOrderDialog">备注订单</el-button>
        </div>
      ...
      <el-table
        ...
        <el-table-column label="出发时间" align="center" v-if="order.businessType !== 3">
          <template slot-scope="scope">
            <p>{{JSON.parse(scope.row.sp3).startDate}}</p>
          </template>
        </el-table-column>
        ...
        <el-table-column label="数量" width="120" align="center">
          <template slot-scope="scope">
            <div v-show="scope.row.childNum">
            {{scope.row.adultNum}}<br/>{{scope.row.childNum}}
            </div>
            <div v-show="!scope.row.childNum">
        ...
        <el-table-column label="小计" width="120" align="center">
          <template slot-scope="scope">
            <div v-if="scope.row.prices">
              {{scope.row.prices | formatTotalPrice}}
            </div>
            <div v-else>
              ￥{{scope.row.productPrice*scope.row.adultNum + (scope.row.productChildPrice?scope.row.productChildPrice:0)*scope.row.childNum}}
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="订单付款"
               :visible.sync="payMoneyDialogVisible"
               width="40%">
      <el-form :model="payInfo"
               label-width="150px">
               <el-form-item label="付款金额：">
          <el-input v-model="payInfo.amount" type="text">
          </el-input>
        </el-form-item>
        <el-form-item label="操作备注：">
          <el-input v-model="payInfo.note" type="textarea" rows="3">
          </el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="材料办理确认"
               :visible.sync="finishStuffDialogVisible"
               width="40%">
      <el-form :model="stuffInfo"
               label-width="150px">
        <el-form-item label="操作备注：">
          <el-input v-model="stuffInfo.note" type="textarea" rows="3">
          </el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="协议签署确认"
               :visible.sync="agreementDialogVisible"
               width="40%">
      <el-form :model="agreeInfo"
               label-width="150px">
        <el-form-item label="操作备注：">
          <el-input v-model="agreeInfo.note" type="textarea" rows="3">
          </el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="订单完成确认"
               :visible.sync="completeOrderDialogVisible"
               width="40%">
      <el-form :model="completeInfo"
               label-width="150px">
        <el-form-item label="操作备注：">
          <el-input v-model="completeInfo.note" type="textarea" rows="3">
          </el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="订单退款确认"
               :visible.sync="refundOrderDialogVisible"
               width="40%">
      <el-form :model="refundInfo"
               label-width="150px">
               <el-form-item label="退款金额：">
          <el-input v-model="refundInfo.amount" type="text">
          </el-input>
        </el-form-item>
        <el-form-item label="操作备注：">
          <el-input v-model="refundInfo.note" type="textarea" rows="3">
          </el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog title="关闭当前订单"
               :visible.sync="closeThisOrderDialogVisible"
               width="40%">
      <el-form :model="closeThisOrderInfo"
               label-width="150px">
        <el-form-item label="操作备注：">
          <el-input v-model="closeThisOrderInfo.note" type="textarea" rows="3">
          </el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
<script>
  import {getOrderDetail,updateReceiverInfo,updateMoneyInfo,closeOrder,updateOrderNote,
        deleteOrder,payMoney,finishStuff,agreementSign,completeOrder,refundOrder,closeThisOrder} from '@/api/order';
  ...
  export default {
    ...
    filters: {
      formatActivityType(row) {
            if (!row.activityType || row.activityType === 0) {
      ...
      formatPrices(value){
        ...
        obj.map((item)=>{
          ...
          str += date[0] + "年" + date[1] + "月" + date[2] + "日" + "：" + item.salePrice + "元 "
        })
        return str
      },
