    ...
    <div class="detail-container">
        <div>
            <el-steps :active="formatStepStatus(order.status)" finish-status="success" align-center>
                ...
                <!-- <el-step title="材料办理" :description="formatTime(order.handleFinishTime)"></el-step>
                <el-step title="协议签署" :description="formatTime(order.signTime)"></el-step>-->
                <el-step title="完成订单" :description="formatTime(order.commentTime)"></el-step>
            </el-steps>
        </div>
        <el-card shadow="never" style="margin-top: 15px">
            <div class="operate-container">
                ...
                <!-- 0 - 待付款；1 - 待材料办理；2 - 待协议签署；3 - 参团成功；4 - 已关闭；6 - 已完成 -->
                <div class="operate-button-container" v-show="order.status===0">
                    <!-- <el-button size="mini" @click="showUpdateReceiverDialog">修改收货人信息</el-button> -->
                    <!-- <el-button size="mini">修改商品信息</el-button> -->
                    <el-button size="mini" @click="showPayMoneyDialog">订单付款</el-button>
                    <!-- <el-button size="mini" @click="showMessageDialog">发送站内信</el-button> -->
                    <!-- <el-button size="mini" @click="showCloseOrderDialog">关闭订单</el-button> -->
                    <el-button size="mini" @click="showCloseThisOrderDialog">关闭当前订单</el-button>
                    <el-button size="mini" @click="showMarkOrderDialog">备注订单</el-button>
                </div>
                <div class="operate-button-container" v-show="order.status===1">
                    <el-button size="mini" @click="showPayMoneyDialog">订单付款</el-button>
                    <el-button size="mini" @click="showCompleteOrderDialog">订单完成确认</el-button>
                    <!-- <el-button size="mini" @click="showUpdateReceiverDialog">修改收货人信息</el-button> -->
                    <!-- <el-button size="mini" @click="showMessageDialog">发送站内信</el-button> -->
                    <el-button size="mini" @click="showRefundOrderDialog">订单退款</el-button>
                    <!-- <el-button size="mini" @click="showCloseThisOrderDialog">关闭当前订单</el-button> -->
                    <el-button size="mini" @click="showMarkOrderDialog">备注订单</el-button>
                </div>
            ...
            <div class="table-layout">
                <el-row>
                    ...
                    <el-col :span="3" class="table-cell-title">订单来源</el-col>
                    <el-col :span="3" class="table-cell-title">订单类型</el-col>
                    <el-col :span="2" class="table-cell-title">是否活动</el-col>
                </el-row>
                <el-row>
                    ...
                    <el-col :span="3" class="table-cell">{{order.sourceType | formatSourceType}}</el-col>
                    <el-col :span="3" class="table-cell">{{order.orderType | formatOrderType}}</el-col>
                    <el-col
                        :span="2"
                        class="table-cell"
                    >{{order.isActivityOrder | formatIsActivityOrder}}</el-col>
                </el-row>
            </div>
            <div style="margin-top: 20px">
                <svg-icon icon-class="marker" style="color: #606266"></svg-icon>
            ...
            >
                ...
                <el-table-column label="商品分类" align="center">
                    <template>
                        <p>{{order | formatActivityType}}</p>
                        <p v-show="order.activityType && order.activityType !== 0">
                            <el-button size="mini" @click="checkActivity(order)">查看活动</el-button>
                        </p>
                    </template>
                </el-table-column>
                <el-table-column label="价格" width="180" align="center">
                    <!-- <template slot-scope="scope">
            <div v-show="scope.row.childNum">
              <p>成人：￥{{scope.row.productPrice}}<br/>儿童：￥{{scope.row.productChildPrice}}</p>
            </div>
            <div v-show="!scope.row.childNum">
              <p>￥{{scope.row.productPrice}}</p>
                    ...
                    <template slot-scope="scope">
                        <div v-if="scope.row.prices">{{scope.row.prices | formatPrices}}</div>
                        <div v-else>
                            <div v-show="scope.row.childNum">
                                <p>
                                    成人：￥{{scope.row.productPrice}}
                                    <br />
                                    儿童：￥{{scope.row.productChildPrice}}
                                </p>
                            </div>
                            <div v-show="!scope.row.childNum">
                                <p>￥{{scope.row.productPrice}}</p>
                ...
                <el-table-column label="数量" width="120" align="center">
                    <template slot-scope="scope">
                        ...
                        <div v-show="!scope.row.childNum">{{scope.row.productQuantity}}</div>
                    </template>
                </el-table-column>
                <el-table-column label="小计" width="120" align="center">
                    <template slot-scope="scope">
                        <div v-if="scope.row.prices">{{scope.row.prices | formatTotalPrice}}</div>
                        <div
                            v-else
                        >￥{{scope.row.productPrice*scope.row.adultNum + (scope.row.productChildPrice?scope.row.productChildPrice:0)*scope.row.childNum}}</div>
                    </template>
                </el-table-column>
            </el-table>
            ...
            <div class="table-layout">
                ...
                <el-row>
                    ...
                    <el-col :span="6" class="table-cell">
                        <span
                            class="color-danger"
                        >￥{{order.payAmount+moneyInfo.freightAmount-moneyInfo.discountAmount}}</span>
                    </el-col>
                </el-row>
            </div>
            <span slot="footer" class="dialog-footer">
        ...
        <el-dialog title="订单付款" :visible.sync="payMoneyDialogVisible" width="40%">
            <el-form :model="payInfo" label-width="150px">
                <el-form-item label="付款金额：">
                    <el-input v-model="payInfo.amount" type="text"></el-input>
                </el-form-item>
                <el-form-item label="操作备注：">
                    <el-input v-model="payInfo.note" type="textarea" rows="3"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="payMoneyDialogVisible = false">取 消</el-button>
                <el-button type="primary" @click="handlePayMoney">确 定</el-button>
            </span>
        </el-dialog>

        <el-dialog title="材料办理确认" :visible.sync="finishStuffDialogVisible" width="40%">
            <el-form :model="stuffInfo" label-width="150px">
                <el-form-item label="操作备注：">
                    <el-input v-model="stuffInfo.note" type="textarea" rows="3"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="finishStuffDialogVisible = false">取 消</el-button>
                <el-button type="primary" @click="handleFinishStuff">确 定</el-button>
            </span>
        </el-dialog>

        <el-dialog title="协议签署确认" :visible.sync="agreementDialogVisible" width="40%">
            <el-form :model="agreeInfo" label-width="150px">
                <el-form-item label="操作备注：">
                    <el-input v-model="agreeInfo.note" type="textarea" rows="3"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="agreementDialogVisible = false">取 消</el-button>
                <el-button type="primary" @click="handleAgreementSign">确 定</el-button>
            </span>
        </el-dialog>

        <el-dialog title="订单完成确认" :visible.sync="completeOrderDialogVisible" width="40%">
            <el-form :model="completeInfo" label-width="150px">
                <el-form-item label="操作备注：">
                    <el-input v-model="completeInfo.note" type="textarea" rows="3"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="completeOrderDialogVisible = false">取 消</el-button>
                <el-button type="primary" @click="handleCompleteOrder">确 定</el-button>
            </span>
        </el-dialog>

        <el-dialog title="订单退款确认" :visible.sync="refundOrderDialogVisible" width="40%">
            <el-form :model="refundInfo" label-width="150px">
                <el-form-item label="退款金额：">
                    <el-input v-model="refundInfo.amount" type="text"></el-input>
                </el-form-item>
                <el-form-item label="操作备注：">
                    <el-input v-model="refundInfo.note" type="textarea" rows="3"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="refundOrderDialogVisible = false">取 消</el-button>
                <el-button type="primary" @click="handleRefundOrder">确 定</el-button>
            </span>
        </el-dialog>
        ...
        <el-dialog title="关闭当前订单" :visible.sync="closeThisOrderDialogVisible" width="40%">
            <el-form :model="closeThisOrderInfo" label-width="150px">
                <el-form-item label="操作备注：">
                    <el-input v-model="closeThisOrderInfo.note" type="textarea" rows="3"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="closeThisOrderDialogVisible = false">取 消</el-button>
                <el-button type="primary" @click="handleCloseThisOrder">确 定</el-button>
            </span>
        </el-dialog>
...
export default {
    ...
    data() {
        return {
            ...
            finishStuffDialogVisible: false,
            agreementDialogVisible: false,
            completeOrderDialogVisible: false,
            ...
            closeThisOrderDialogVisible: false,
            closeInfo: { note: null, id: null },
            payInfo: { amount: null, note: null },
            stuffInfo: { note: null },
            agreeInfo: { note: null },
            signInfo: { note: null },
            completeInfo: { note: null },
            refundInfo: { note: null, amount: null },
            closeThisOrderInfo: { note: null },
    ...
    filters: {
        formatActivityType(row) {
            if (row.activityType === null || row.activityType === 0) {
                let value = row.businessType;
                if (value === 1) {
                    return "旅游产品";
                } else if (value === 2) {
                    return "机票产品";
        ...
        formatPrices(value) {
            ...
            let str = "";
            obj.map(item => {
                let date = item.arrivalDate.split("-");
                str +=
                    date[0] +
                    "年" +
                    date[1] +
        ...
        formatStatus(value) {
            // 0 - 待付款；1 - 待材料办理；2 - 待协议签署；3 - 参团成功；4 - 已关闭；6 - 已完成；7 - 已退款
            if (value === 1) {
                return "已付款";
            } else if (value === 2) {
                return "待协议签署";
    ...
    methods: {
        checkActivity(order) {
            if (order.activityType === 1) {
                this.$router.push({
                    path: "/sms/examine",
                    query: { id: order.activityId }
                });
            } else {
                getActivityIdByJoinId(order.activityId).then(response => {
                    this.$router.push({
        ...
        showPayMoneyDialog() {
            this.payMoneyDialogVisible = true;
            this.payInfo.amount = null;
            this.payInfo.note = null;
        },
        showHandleFinishStuffDialog() {
            this.finishStuffDialogVisible = true;
            this.stuffInfo.note = null;
        },
        showAgreementSignDialog() {
            this.agreementDialogVisible = true;
            this.agreeInfo.note = null;
        },
        showCompleteOrderDialog() {
            this.completeOrderDialogVisible = true;
            this.completeInfo.note = null;
        },
        showRefundOrderDialog() {
            this.refundOrderDialogVisible = true;
            this.refundInfo.note = null;
            this.refundInfo.amount = this.order.payAmount;
        },
        ...
        handlePayMoney() {
            this.$confirm("是否要添加付款?", "提示", {
                ...
                let data = {
                    ...
                    note: this.payInfo.note,
                    paidMoney: Number(this.payInfo.amount)
                };
                payMoney(data).then(response => {
                    this.payMoneyDialogVisible = false;
                    this.$message({
                        type: "success",
                        message: "订单付款成功!"
                    });
        ...
        handleFinishStuff() {
            this.$confirm("是否要确认材料办理?", "提示", {
                ...
                let data = {
                    orderId: Number(this.id),
                    note: this.stuffInfo.note
                };
                finishStuff(data).then(response => {
                    this.finishStuffDialogVisible = false;
                    this.$message({
                        type: "success",
                        message: "认材料办理成功!"
                    });
        ...
        handleAgreementSign() {
            this.$confirm("是否要确认协议签署?", "提示", {
                ...
                let data = {
                    orderId: Number(this.id),
                    note: this.signInfo.note
                };
                agreementSign(data).then(response => {
        ...
        handleCompleteOrder() {
            this.$confirm("是否要确认订单完成?", "提示", {
                ...
                let data = {
                    orderId: Number(this.id),
                    note: this.completeInfo.note
                };
                completeOrder(data).then(response => {
        ...
        handleRefundOrder() {
            this.$confirm("是否要确认订单退款?", "提示", {
                ...
                let data = {
                    orderId: Number(this.id),
                    refundPrice: Number(this.refundInfo.amount),
                    note: this.refundInfo.note
                };
        ...
        showCloseThisOrderDialog() {
            this.closeThisOrderDialogVisible = true;
            this.closeThisOrderInfo.note = null;
        },
        handleCloseOrder() {
        ...
        handleCloseThisOrder() {
            this.$confirm("是否要关闭当前订单?", "提示", {
                confirmButtonText: "确定",
                cancelButtonText: "取消",
                type: "warning"
            }).then(() => {
                ...
                closeThisOrder(data).then(response => {
                    this.closeThisOrderDialogVisible = false;
                    this.$message({
                        type: "success",
                        message: "当前订单关闭成功!"
                    });
